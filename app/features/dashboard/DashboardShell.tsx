...
export function DashboardShell({orgId}: {orgId: string | null}) {
  ...
  const tickets: QueueTicket[] = realTickets.map(mapRealToQueue);
  
  console.log("This is realTickets:", realTickets)
  const openTickets = tickets.filter((t) => t.status !== "resolved");
  const resolvedTickets = tickets.filter((t) => t.status === "resolved");
  const displayTickets = activeTab === "open" ? openTickets : resolvedTickets;


  const effectiveSelectedId = selectedId || displayTickets[0]?.id || null;
  const selectedTicket = displayTickets.find((t) => t.id === effectiveSelectedId);
      ...
      <aside className="w-full sm:w-80 lg:w-96 border-r border-charcoal-700 flex-shrink-0 flex flex-col">
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex items-center justify-center px-4">
            <p className="text-sm text-accent-400">Failed to load tickets. Please reload your browser.</p>
          </div>
        ) : (
          <TicketQueue
